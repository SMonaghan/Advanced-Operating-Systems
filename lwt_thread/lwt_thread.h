...
#define STACK_SIZE (16 * 1024)//Size of the stack
...
//unsigned int * count = mmap(NULL, sizeof(unsigned int), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
...
lwt_t lwt_create(lwt_fn_t fn, void *data){
	...
	lwt_thd->mem = malloc(STACK_SIZE);
	lwt_thd->id = generate_id();
	lwt_thd->sp = (unsigned int)&lwt_thd->mem;
